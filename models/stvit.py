...
import time
# https://blog.csdn.net/allrubots/article/details/130617255?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522ef5ce675c58489f88e4d4497adc0b79a%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=ef5ce675c58489f88e4d4497adc0b79a&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduend~default-1-130617255-null-null.142^v102^pc_search_result_base7&utm_term=resdwc&spm=1018.2226.3001.4449
...
"""
    PyTorch 的 nn.LayerNorm 默认作用于最后一个维度，但我们这里输入是 (B, C, H, W)，
    所以需要先将 C 放到最后进行 LayerNorm，再放回来。
    这个类定义的是一个适用于 4D 张量（比如图像）的 LayerNorm 版本，
    其核心功能是对每个空间位置（H×W）上的通道维（C）进行 Layer Normalization（层归一化）。
    
    eps=1e-6：为了数值稳定性，防止除以 0。    https://blog.csdn.net/qq_37541097/article/details/117653177?ops_request_misc=%257B%2522request%255Fid%2522%253A%25225d0c8a660908a917fd1231a9cfefdaaa%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fblog.%2522%257D&request_id=5d0c8a660908a917fd1231a9cfefdaaa&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~blog~first_rank_ecpm_v1~rank_v31_ecpm-1-117653177-null-null.nonecase&utm_term=layer&spm=1018.2226.3001.4450
    permute(0,2,3,1) 交换维度顺序  （B, C, H, W） -> (B, H, W, C)
    contiguous()：重新排列后可能不是连续内存，需要这个方法以确保后续兼容。
"""
...
"""
    带残差的深度可分离卷积模块
    实现了残差连接 + 深度卷积（Depthwise Convolution），
    仅对每个通道独立卷积，然后与输入相加形成残差路径。这种设计可以保留输入信息同时引入局部特征。
    https://blog.csdn.net/m0_37605642/article/details/134174749?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522197d5b9b434a9f4f826b66e527673b6e%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=197d5b9b434a9f4f826b66e527673b6e&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-1-134174749-null-null.142^v102^pc_search_result_base7&utm_term=dwconv&spm=1018.2226.3001.4449
"""
...
@register_model
def stvit_base(args):
    model = STViT(embed_dim=[96, 192, 384, 512],  # 52M, 9.9G, 361 FPS      输入 PatchEmbed 中的 out_channel 对应结构图中的前 四次卷积 的 out_channel
                  depths=[4, 6, 14, 6],
                  num_heads=[2, 3, 6, 8],
                  n_iter=[1, 1, 1, 1],
                  stoken_size=[8, 4, 1, 1],
...
def test():
    model = STViT(
        embed_dim=[96, 192, 448, 640],  # 95M, 15.6G, 269 FPS   输入
        depths=[4, 7, 19, 8],
        num_heads=[2, 3, 7, 10],
